...
function validateTextDocument(textDocument: TextDocument): void {
	...
	let rBlockStart = new RegExp("(^| )(begin case$|(if|evaluate|readnext|open|read|readv|readu|readt|locate|openseq|matread|create|readlist|openpath|find|findstr)\\s+?)", "i");
...
connection.onDidChangeConfiguration(change => {
	...
	maxNumberOfProblems = settings.Linkar.maxNumberOfProblems || 100;
	useCamelcase = settings.Linkar.useCamelCase;
	ignoreGotoScope = settings.Linkar.ignoreGotoScope;
	customWordList = settings.Linkar.customWords;
	customWordPath = settings.Linkar.customWordPath;
	languageType = settings.Linkar.languageType;
	const _logLevel = <string>(settings.Linkar.trace && settings.Linkar.trace.server) || 'off';
	switch (_logLevel) {
...
connection.onReferences(params => {
	...
	if (doc) {
		...
		let word = line.substring(labelStart, xpos);
