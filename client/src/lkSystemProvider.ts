...
export class LkFileSystemProvider implements vscode.FileSystemProvider {
    ...
    constructor(conn: Connection) {
       ...
       /*vscode.workspace.onWillSaveTextDocument(event => {
            if (event.document.uri.scheme == this.connection.scheme) {
                //Allow vscode compare with las version of file
                var confs = vscode.workspace.getConfiguration();
                var conflicts = confs.get('files.saveConflictResolution') as string;
                if (conflicts == "askUser"){
                    this.FillFile(event.document.uri, true);
                }
            }
        });*/
    }
    ...
    FillFile(uri: vscode.Uri, fromOpenResource: boolean): void {
        ...
        //Avoid reload if document is dirty
        var canRead = true;
        for (var editor of vscode.window.visibleTextEditors) {
            if (editor.document.uri.scheme == uri.scheme && editor.document.uri.fsPath == uri.fsPath && editor.document.isDirty) {
                canRead = false;
                break;
            }
        }
    ...
    writeFile(uri: vscode.Uri, content: Uint8Array, options: { create: boolean, overwrite: boolean, isNew: boolean, onDemand: boolean }): void {
        ...
        if (entry) {
            ...
            if (entry.isNew) {
                ...
                var originalRecordEditor = false;
                for (var editor of vscode.window.visibleTextEditors) {
                    var fakeUri = "/" + uri.scheme + uri.path;
                    if (editor.document.uri.path == fakeUri) {
                        var orig = editor.document.getText();
                        orig = orig.replace(new RegExp('\r\n', 'gi'), '\xFE');
                        orig = orig.replace(new RegExp('\r', 'gi'), '\xFE');
                        orig = orig.replace(new RegExp('\n', 'gi'), '\xFE');
                        originalRecord = orig;
                        originalRecordEditor = true;
                        break;
                    }
                }
        ...
        if (error) {            
            if (updateError)
            {
                ...
                //throw new vscode.FileSystemError(error);
                //throw vscode.FileSystemError.FileNotFound(error);
            }
            else
...
export class ReadOnlyFileSystemProvider implements vscode.FileSystemProvider{
    ...
    constructor(schema: string) {
        ...
        /*vscode.workspace.onDidOpenTextDocument(event => {
            if (event.uri.scheme == this.sch) {
                let entry = this._lookup(event.uri, false);
                if (entry) {
                    //if (event.languageId != "mvbasic" && event.languageId == "plaintext")
                    //    vscode.languages.setTextDocumentLanguage(event, "mvbasic");
                    vscode.languages.setTextDocumentLanguage(event, "mvbasic_readonly");
                }                
            }
            return event;
        });*/
